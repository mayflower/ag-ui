...
interface HttpAgentConfig extends AgentConfig {
  ...
  connectUrl?: string // Optional override for the connect endpoint URL
...
const agent = new HttpAgent({
  ...
  // Optional: override connect endpoint (defaults to `${url}/connect`)
  connectUrl: "https://api.example.com/v1/agent/connect",
  headers: {
...
Establishes a persistent connection to load existing thread state and messages.
This sends a request to `connectUrl` if provided, otherwise `${url}/connect`,
and expects an AG-UI event stream (e.g. `MESSAGES_SNAPSHOT`).
